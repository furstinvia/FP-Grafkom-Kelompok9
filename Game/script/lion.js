...
function Chicken() {
  ...
  // Front left leg
  const frontLeftLeg = new THREE.Mesh(new THREE.CylinderBufferGeometry(legRadius, legRadius, legHeight, 8), legMaterial);
  frontLeftLeg.position.set(-baseSize / 4, -baseSize / 2, 0);
  frontLeftLeg.rotation.x = Math.PI / 2;
  lion.add(frontLeftLeg);

  // Front right leg
  const frontRightLeg = new THREE.Mesh(new THREE.CylinderBufferGeometry(legRadius, legRadius, legHeight, 8), legMaterial);
  frontRightLeg.position.set(baseSize / 4, -baseSize / 2, 0);
  frontRightLeg.rotation.x = Math.PI / 2;
  lion.add(frontRightLeg);

  // Back left leg
  const backLeftLeg = new THREE.Mesh(new THREE.CylinderBufferGeometry(legRadius, legRadius, legHeight, 8), legMaterial);
  backLeftLeg.position.set(-baseSize / 4, -baseSize, -10 * heightFactor * zoom);
  backLeftLeg.rotation.x = Math.PI / 2;
  lion.add(backLeftLeg);

  // Back right leg
  const backRightLeg = new THREE.Mesh(new THREE.CylinderBufferGeometry(legRadius, legRadius, legHeight, 8), legMaterial);
